...
In order to ensure proper cleanup of such objects, you need to accommodate for it on a `Flux`-by-`Flux` basis, as well as in several of the global hooks (see <<hooks>>):
...
For example, `bufferWhen` can introduce overlapping buffers, and that means that the discard "local hook" above might see a first buffer as being discarded and cleanup an element in it that is in a second buffer _which is still valid_.
...
Unlike the `doOnDiscard` operator, which performs a class `instanceOf` check, the global hooks are also dealing with instances that can be any `Object` and it is up to the user's implementation to distinguish between which instances need cleanup and which don't.


=== The `doOnDiscard` operator / local hook
...
WARNING: Each call to `doOnDiscard(Class, Consumer)` is additive with the others, to the extent that it is only visible and used by operators upstream of it.
